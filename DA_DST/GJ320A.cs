...
namespace DA_DST
{
    ...
    class GJ320A : NCMachine
    {
        int iNCIndex = -1;
        public GJ320A(string _machnum, string _ip, int _port, int _itype)
        ...
        public override int ConnectToNC(string ip, string username, string password)
        {
            ...
            try
            {
                ret = GoldingA.connectToNC(ip, (ushort)port, ref iNCIndex);
                if (ret == 0)
                {
        ...
        public override int DisConnectToNC()
        {
            ...
            try
            {
                err = GoldingA.disconnectToNC(iNCIndex);
        ...
        public override int ReConnectToNC(string ip, string username, string password)
        {
            ...
            try
            {
                err = GoldingA.disconnectToNC(iNCIndex);
                Thread.Sleep(200);
                err = GoldingA.connectToNC(ip, (ushort)port, ref iNCIndex);    //连接机器
        ...
        public override int UpdateData()
        {
            ...
            int ret = -1;
            int index = -1;
            GOLDINGA.GoldingA.DNC_TOOL_LIFE_TABLE toolLifeTable = new GOLDINGA.GoldingA.DNC_TOOL_LIFE_TABLE();
            GOLDINGA.GoldingA.DNC_TOOLOFFSET_STRUCT tooloffset = new GOLDINGA.GoldingA.DNC_TOOLOFFSET_STRUCT();
            if (0 != UpdateComm())
                ret = -1;
            if (status == MachStatus.MS_ONLINE)
            {
                try
                {
                    ...
                    index = 2001;   //配置轴数
                    if (0 == GoldingA.getStatusIntVal(iNCIndex, 0, index, ref retValueInt))
                    {
                        this.mach_info.axis_num = retValueInt;
                    }

                    index = 1004;//运行状态
                    if (0 == GoldingA.getStatusIntVal(iNCIndex, 0, index, ref retValueInt))//0：空闲1：运行2：暂停3：保持
                    {
                    ...
                    if (0 == GoldingA.getStatusAxisVal(iNCIndex, 0, index, retValueArrayDouble))
                    ...
                    index = 8005;//主轴倍率
                    if (0 == GoldingA.getStatusDoubleVal(iNCIndex, 0, index, ref retValueDouble))
                    ...
                    ////for (int p = 0; p < 10; p++)
                    ////{
                    ////    if (0 == GoldingA.getToolOffsetTable(iNCIndex, 0, p, ref tooloffset))
                    ////    {
                    ...
                    if (0 == GoldingA.getStatusStrVal(iNCIndex, 0, index, buf1))
                    ...
                    if (0 == GoldingA.getErrorVal(iNCIndex, 0, buf2))    //手动清除类故障           
                        this.mach_info.error_handle = buf2.ToString();
                    StringBuilder buf3 = new StringBuilder(256);
                    if (0 == GoldingA.getAutoMotionErrorVal(iNCIndex, 0, buf3))//自动清除类故障
                        this.mach_info.error_motion = buf3.ToString();
                    StringBuilder buf5 = new StringBuilder(256);
                    if (0 == GoldingA.getAutoPlcErrorVal(iNCIndex, 0, buf5))//PLC清除类故障，此部分采集的值仍然为空消息，但已经添加了msg
                        this.mach_info.error_plc = buf5.ToString();
                    if (0 == GoldingA.getToolLifeTable(iNCIndex, 0, 2, ref  toolLifeTable))//刀具管理
                    {
